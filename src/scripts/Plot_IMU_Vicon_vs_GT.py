...
axs[0].plot(ts_gt + 0.2, a_gt_sim[:, 0], "b-", alpha=0.55, label = "X - Simulated")
axs[0].legend()
axs[1].plot(ts_real, a_real_raw[:, 1], "r-", alpha=0.6, label = "Y - Real")
axs[1].plot(ts_gt + 0.2, a_gt_sim[:, 1], "b-", alpha=0.55, label = "Y - Simulated")
axs[1].legend()
axs[2].plot(ts_real, a_real_raw[:, 2], "r-", alpha=0.6, label = "Z - Real")
axs[2].plot(ts_gt + 0.2, a_gt_sim[:, 2], "b-", alpha=0.55, label = "Z - Simulated")
axs[2].legend()
plt.xlabel('t (s)')
plt.show()
...
axs[0].plot(ts_gt + 0.2, w_gt_sim[:, 0], "b-", alpha=0.75, label = "X - Simulated")
axs[0].legend()
axs[1].plot(ts_real, w_real_raw[:, 1], "r-", alpha=0.6, label = "Y - Real")
axs[1].plot(ts_gt + 0.2, w_gt_sim[:, 1], "b-", alpha=0.75, label = "Y - Simulated")
axs[1].legend()
axs[2].plot(ts_real, w_real_raw[:, 2], "r-", alpha=0.6, label = "Z - Real")
axs[2].plot(ts_gt + 0.2, w_gt_sim[:, 2], "b-", alpha=0.75, label = "Z - Simulated")
axs[2].legend()
plt.xlabel('t (s)')
plt.show()

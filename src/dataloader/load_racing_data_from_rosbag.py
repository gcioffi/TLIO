...
def perturbationIMUandBiases(config_fn):
        ...
        for (topic, msg, ts) in bag.read_messages():
            if topic == topic_imu:
                ...
                if first:
                    ...
                    dt_sqrt_ = math.sqrt(dt/2)
                    t = prev_ts_imu + dt / 2.0

                    #IMU Interpolation: measurements will be now at 1000 Hz
    ...
    for i in range(n_trajectories):
        ...
        #Find w and a: calib (GT + IMU noise) and raw (GT + bias + IMU noise + Bias noise)
        ...
        biasesValues = np.concatenate((bias_acc,bias_w), axis=1)
         ...
         #check how are we concatenating

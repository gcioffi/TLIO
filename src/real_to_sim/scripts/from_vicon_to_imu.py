...
if __name__ == '__main__':
    ...
    # transform poses
    T_MB = T_MC * T_CB # T from IMU to Markers
    ...
    for s in markers_states:
        ...
        T_VM = pose.Pose(R_VM, p_VM.reshape(3,1)) # T from Vicon to Markers

        rz, ry, rx = tf.euler_from_matrix(R_VM, 'rzyx')
        markers_ori_zyx.append(np.array([rz, ry, rx]))

        T_VB = T_VM * T_MB # IMU to Vicon
        T_VB.fix()
        q_VB = T_VB.q_wxyz()
        p_VB = T_VB.t.flatten()
    ...
    outfn = os.path.join(os.path.dirname(args.ev_state_fn), 'evolving_state.txt')
    ...
    plt.figure('Velocity')
    plt.subplot(311)
    plt.plot(t, imu_states[:, 8], label='body')
    plt.plot(t, markers_states[:, 8], label='markers')
    plt.grid()
    plt.legend()
    plt.xlabel('t')
    plt.ylabel('x')
    ...
    plt.subplot(312)
    plt.plot(t, imu_states[:, 9], label='body')
    plt.plot(t, markers_states[:, 9], label='markers')
    plt.grid()
    plt.legend()
    plt.xlabel('t')
    plt.ylabel('y')
    ...
    plt.subplot(313)
    plt.plot(t, imu_states[:, 10], label='body')
    plt.plot(t, markers_states[:, 10], label='markers')
    plt.grid()
    plt.legend()
    plt.xlabel('t')
    plt.ylabel('z')

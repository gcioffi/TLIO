...
axs[0].plot(ts_sim - 0.2, v_wb_sim[:, 0], "b-", alpha=0.75, label = "vx - SIM")
axs[0].legend()
axs[0].grid()
axs[1].plot(ts_real, v_wb_y, "r-", alpha=0.6, label = "vy - VICON")
axs[1].plot(ts_sim - 0.2, v_wb_sim[:, 1], "b-", alpha=0.75, label = "vy - SIM")
axs[1].legend()
axs[1].grid()
axs[2].plot(ts_real, v_wb_z, "r-", alpha=0.6, label = "vz - VICON")
axs[2].plot(ts_sim - 0.2, v_wb_sim[:, 2], "b-", alpha=0.75, label = "vz - SIM")
axs[2].legend()
axs[2].grid()
plt.xlabel('t (s)')
plt.show()
...
axs[0].plot(ts_sim - 0.2, p_wb_sim[:, 0], "b-", alpha=0.75, label = "x - SIM")
axs[0].legend()
axs[0].grid()
axs[1].plot(ts_real, p_wb_y, "r-", alpha=0.6, label = "y - VICON")
axs[1].plot(ts_sim - 0.2, p_wb_sim[:, 1], "b-", alpha=0.75, label = "y - SIM")
axs[1].legend()
axs[1].grid()
axs[2].plot(ts_real, p_wb_z, "r-", alpha=0.6, label = "z - VICON")
axs[2].plot(ts_sim - 0.2, p_wb_sim[:, 2], "b-", alpha=0.75, label = "z - SIM")
axs[2].legend()
axs[2].grid()

...
axs[0].plot(ts_gt, v_wb_gt[:, 0], "b-", alpha=0.75, label = "vx - GT")
axs[0].legend()
axs[0].grid()
axs[1].plot(ts_real, v_wb_y, "r-", alpha=0.6, label = "vy - VICON")
axs[1].plot(ts_gt, v_wb_gt[:, 1], "b-", alpha=0.75, label = "vy - GT")
axs[1].legend()
axs[1].grid()
axs[2].plot(ts_real, v_wb_z, "r-", alpha=0.6, label = "vz - VICON")
axs[2].plot(ts_gt, v_wb_gt[:, 2], "b-", alpha=0.75, label = "vz - GT")
axs[2].legend()
axs[2].grid()
plt.xlabel('t (s)')
plt.show()
...
axs[0].plot(ts_gt, p_wb_gt[:, 0], "b-", alpha=0.75, label = "x - GT")
axs[0].legend()
axs[0].grid()
axs[1].plot(ts_real, p_wb_y, "r-", alpha=0.6, label = "y - VICON")
axs[1].plot(ts_gt, p_wb_gt[:, 1], "b-", alpha=0.75, label = "y - GT")
axs[1].legend()
axs[1].grid()
axs[2].plot(ts_real, p_wb_z, "r-", alpha=0.6, label = "z - VICON")
axs[2].plot(ts_gt, p_wb_gt[:, 2], "b-", alpha=0.75, label = "z - GT")
axs[2].legend()
axs[2].grid()

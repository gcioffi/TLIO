...
#load_racing_data_from_rosbag

This file loads a trajectory-rosbag and generates different versions of it perturbing biases and noises on both IMU and biases. 
Angular velocity and acceleration GT (from simulation) are imported, whereas calibrated (with noise on the IMU) and raw angular velocity and acceleration are generated. 
...
Launching gen_racing_data.py, it is then possible to get the hdf5 file needed for the training step. When launching this script, a data directory --data_dir should be specified: TLIO/data/Dataset. 

#gen_racing_data.py

- obtain raw and vio-calibrated IMU data
- obtain vio states by integrating vio-calibrated IMU
 
# Parameters

A folder called "params" is generated in the directory: TLIO/src. In this folder, there is a yaml file called "dataloader_params.yaml" used to load the bagfile with the trajectory to train, the topics recorded (IMU + Odometry), the number of trajectories generated by "load_racing_data_from_rosbag" and the output directory in which the dataset will be generated. Furthermore, in this config file there are also the standard deviations values for the IMU and bias noises on acceleration and angular velocity and the bias values.
When launching this script, a --config directory should be specified: TLIO/src/params/dataloader_params.yaml

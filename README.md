...
If SNAGA is used, instead, it is necessary to use dataloader_params_snaga. 

## load_racing_data_from_rosbag

"load_racing_data_from_rosbag" loads a rosbag of a certain trajectory and generates different versions of it perturbing biases and noises on both IMU and biases. The number of versions is specified by "n_trajectories: 2" in the "dataloader_params.yaml". 
...
Launching "gen_racing_data.py", it is possible to get the hdf5 file needed for the training step. When launching this script, a data directory --data_dir should be specified: TLIO/data/Dataset. 
